...
!       INTERSECTION WITH HULL IS PERFOMED		!
...
DATA XXI /0.0  , 0.005, 0.0125, 0.025, 0.050, 0.075, 0.100,&
         ...
         &0.700, 0.800, 0.9000, 0.950, 1.000/
...
1 FORMAT(//2X,'TRAILING EDGE LOCATED AT X = ',F10.5)
  DX = 0.05
  X  =HH+DX
  WRITE(FILENAME,'(A14,I1)') "appendage-case",K
...
!-------------------------------!
      ...
      SRS = RR*RR + Z*Z
!-------------------------------!
!    IF APPENDAGE LOCATED       !
!    INSIDE BODY, INCREASE R    !
!-------------------------------!
      IF(SRS.LT.RHAS .AND. ITR.EQ.0) GOTO 700
!-------------------------------!
!    IF APPENDAGE LOCATED       !
!    ON BODY SURFACE,   	!
!    GOTO 710 TO DEFINE SECTION !
!-------------------------------!
      IF(ABS(SRS-RHAS).LE.0.00001) GOTO 710
!-------------------------------!
!    APPENDAGE IS CLOSE TO HULL !
!    RADIUS, GET CLOSER         !
!-------------------------------!
      ITR = ITR + 1
      IF(ITR.GT.20) STOP 1
